...
public class JFXButtonSkin extends ButtonSkin
{
   ...
   protected void layoutChildren(final double x, final double y, final double w, final double h)
   {
         ...
         if (((JFXButton) getSkinnable()).getRipplerFill() == null)
         {
            ...
            for (int i = getChildren().size() - 1; i >= 1; i--)
            {
               if (getChildren().get(i) instanceof Text)
               {
                  ...
                  ((Text) getChildren().get(i)).fillProperty().addListener((o, oldVal, newVal) -> buttonRippler.setRipplerFill(newVal));
                  break;
               }
      ...
      buttonRippler.resizeRelocate(getSkinnable().getLayoutBounds().getMinX(),

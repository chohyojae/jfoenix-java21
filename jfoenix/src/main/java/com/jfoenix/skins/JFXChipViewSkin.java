...
public class JFXChipViewSkin<T> extends SkinBase<JFXChipView<T>>
{
   ...
   private class CustomFlowPane extends FlowPane
   {
      ...
      public void updateEditorPosition()
      {
         ...
         {
            layoutInArea(editor, newLineEditorX, top, insideWidth - initOffset, editor.prefHeight(-1), 0, getColumnHAlignmentInternal(), VPos.TOP);
            editorOnNewLine = true;
            ensureVisible(editor);
         }
      }
      ...
      public void show(Node node)
      {
         ...
         if (!isShowing())
         {
            ...
            this.show(parent,
                      ...
                      parent.getY() + node.localToScene(0, 0).getY() + node.getScene().getY() + node.getLayoutBounds().getHeight() + shift);

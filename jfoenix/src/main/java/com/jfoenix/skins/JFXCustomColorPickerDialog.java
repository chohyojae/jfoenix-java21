...
public class JFXCustomColorPickerDialog extends StackPane
{
   ...
   public JFXCustomColorPickerDialog(Window owner)
   {
      ...
      initRun = () ->
      {
         ...
         pane.backgroundProperty().addListener((o, oldVal, newVal) ->
         {
            ...
            {
               Color fontColor = ((Color) newVal.getFills().get(0).getFill()).grayscale().getRed() > 0.5 ? Color.valueOf("rgba(40, 40, 40, 0.87)")
         ...
         Platform.runLater(() ->
         {
            pane.setBackground(new Background(new BackgroundFill(curvedColorPicker.getColor(curvedColorPicker.getSelectedIndex()),
                                                                 CornerRadii.EMPTY,
                                                                 Insets.EMPTY)));
            ((Region) tabs.lookup(".tab-header-background")).setBackground(new Background(new BackgroundFill(curvedColorPicker.getColor(curvedColorPicker.getSelectedIndex()),
                                                                                                             CornerRadii.EMPTY,
                                                                                                             Insets.EMPTY)));
            Region tabsHeader = (Region) tabs.lookup(".tab-header-background");
            pane.backgroundProperty().unbind();
            ...
            {
               return new Background(new BackgroundFill(pane.getBackground().getFills().get(0).getFill(), CornerRadii.EMPTY, Insets.EMPTY));
            }, pane.backgroundProperty()));

            ((Pane) pickerDecorator.lookup(".jfx-decorator-content-container")).borderProperty().bind(Bindings.createObjectBinding(() ->
            {
               return new Border(new BorderStroke(pane.getBackground().getFills().get(0).getFill(),
   ...
   public void show()
   {
      ...
      Timeline timeline = new Timeline(new KeyFrame(Duration.millis(120), new KeyValue(dialog.opacityProperty(), 1, Interpolator.EASE_BOTH)));

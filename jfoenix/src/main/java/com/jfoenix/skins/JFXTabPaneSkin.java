...
public class JFXTabPaneSkin extends SkinBase<JFXTabPane>
{
   ...
   /*
    * keep track of indices after changing the tabs, it used to fix tabs animation after changing the
    * tabs (remove/add)
    */
   ...
   protected class HeaderContainer extends StackPane
   {
         ...
         headersRegion = new StackPane()
         {
            ...
            protected void layoutChildren()
            {
               ...
               else
               {
                  ...
                  {
                     double offset = 0;
                     double w = header.getWidth() - snapSize(rightControlButton.prefWidth(-1)) - snapSize(leftControlButton.prefWidth(-1)) - snappedLeftInset()
                                - SPACER;
               ...
               selectedTabLine.resizeRelocate((side == Side.LEFT || side == Side.BOTTOM) ? snapSize(headersRegion.getWidth()) : 0,
      ...
      {
         if (selectedTabLine.getTranslateX() == newTransX && scale.getX() == newWidth)
            return;
         ...
         double midScaleX = tempScaleX != 0 ? tempScaleX : ((Math.abs(transDiff) / translateScaleFactor + oldWidth) * oldScaleX) / oldWidth;
      ...
      protected void layoutChildren()
      {
         ...
         positionInArea(leftControlButton,
                        (side == Side.LEFT || side == Side.BOTTOM) ? w - btnWidth : 0,
                        controlStartY,
                        btnWidth,
                        btnHeight,
                        0,
   ...
   protected class TabHeaderContainer extends StackPane
   {
      ...
      {
         return tab.isClosable() && (getSkinnable().getTabClosingPolicy().equals(TabPane.TabClosingPolicy.ALL_TABS));
         //                   ||

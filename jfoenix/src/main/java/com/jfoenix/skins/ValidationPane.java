...
public class ValidationPane<T extends Region & IFXValidatableControl & IFXStaticControl> extends HBox
{
   ...
   private Timeline errorHideTransition = new Timeline(new KeyFrame(Duration.millis(80), new KeyValue(opacityProperty(), 0, Interpolator.LINEAR)));
   private Timeline errorShowTransition = new Timeline(new KeyFrame(Duration.millis(80), new KeyValue(opacityProperty(), 1, Interpolator.EASE_OUT)));
      ...
      control.activeValidatorProperty().addListener((ObservableValue<? extends ValidatorBase> o, ValidatorBase oldVal, ValidatorBase newVal) ->
      {
         if (!control.isDisableAnimation())
         {
               ...
               errorHideTransition.setOnFinished(finish ->
               {
                  ...
                  if (errorLabel.isWrapText())
                  {
                     ...
                     if (errorContainerHeight < getHeight())
                     {
                        ...
                        SequentialTransition transition = new SequentialTransition(scaleLess1, errorShowTransition);
                        transition.play();
                     }
                     else
                     {
            ...
            else
            {
               ...
               {
                  // animate scale only
                  scaleLess1.getKeyFrames().setAll(new KeyFrame(Duration.millis(100), new KeyValue(errorClipScale.yProperty(), 0, Interpolator.EASE_BOTH)));
                  scaleLess1.setOnFinished(event ->
                  {
                     hideError();
   ...
   {
      return isVisible() && errorShowTransition.getStatus().equals(Animation.Status.STOPPED)

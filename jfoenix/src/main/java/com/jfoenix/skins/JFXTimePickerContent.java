...
public class JFXTimePickerContent extends VBox
{
   ...
   private ObjectProperty<Rotate> pointerRotate = new SimpleObjectProperty<>(), _24HourPointerRotate = new SimpleObjectProperty<>();
   ...
   {
      Circle circle = new Circle(contentCircleRadius), selectionCircle = new Circle(contentCircleRadius / 6);
      ...
      EventHandler<? super MouseEvent> mouseActionHandler = (event) ->
      {
         ...
         int index = (int) Math.round((180 + Math.toDegrees(theta)) / angle.get()), timeValue;
         if (_24HourView)
         {
   ...
   /*
    * header panel represents the selected Time we keep javaFX original style classes
    */
   ...
   private StackPane createHoursContent(LocalTime time, boolean _24HourView)
   {
      ...
      for (int i = 0; i < 12; i++)
      {
         ...
         // init color
         label.setTextFill(((val == time.getHour() % 12 || (val == 12 && time.getHour() % 12 == 0)) && !is24HourView) ? Color.rgb(255, 255, 255, 0.87)
      ...
      if (_24HourView)
      {
         ...
         for (int i = 0; i < 12; i++)
         {
            ...
            // init color
            label.setTextFill((val == time.getHour() % 24 || (val == 0 && time.getHour() % 24 == 0) && is24HourView) ? Color.rgb(255, 255, 255, 0.54)
   ...
   void updateValue()
   {
      ...
      {
         timePicker.setValue(LocalTime.parse(selectedHourLabel.getText() + ":" + selectedMinLabel.getText() + " " + period.get(),

...
/**
 * <h1>Generic Editable Tree Table cell</h1> Provides the base for an editable table cell using a
 * text field. Sub-classes can provide formatters for display and a commitHelper to control when
...
public class GenericEditableTreeTableCell<S, T> extends JFXTreeTableCell<S, T>
{
   ...
   /**
    * constructor that creates the default
    * {@link com.jfoenix.controls.cells.editors.TextFieldEditorBuilder TextField} editor node to edit
    * the cell
    */
   ...
   /**
    * Any action attempting to commit an edit should call this method rather than commit the edit
    * directly itself. This method will perform any validation and conversion required on the value.
    * For text values that normally means this method just commits the edit but for numeric values, for
    * example, it may first parse the given input.
    * <p>
    * The only situation that needs to be treated specially is when the field is losing focus. If you
    * user hits enter to commit the cell with bad data we can happily cancel the commit and force them
    * to enter a real value. If they click away from the cell though we want to give them their old
    * value back.
    *
   ...
   /**
    * @param forward true gets the column to the right, false the column to the left of the current
    *                column
    * @return
    */

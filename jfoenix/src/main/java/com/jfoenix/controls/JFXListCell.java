 ...
 * <p>
 * By default JFXListCell will try to create a graphic node for the cell, to override it you need to
 * set graphic to null in {@link #updateItem(Object, boolean)} method.
 * <p>
 * NOTE: passive nodes (Labels and Shapes) will be set to mouse transparent in order to show the
 * ripple effect upon clicking , to change this behavior you can override the method
 * {{@link #makeChildrenTransparent()}
 *
...
public class JFXListCell<T> extends ListCell<T>
{
   ...
   /**
    * this method is used to set some nodes in cell content as mouse transparent nodes so clicking on
    * them will trigger the ripple effect.
    */
   ...
   {
      return (getListView() instanceof JFXListView) ? (((JFXListView<?>) getListView()).isExpanded() ? ((JFXListView<?>) getListView()).currentVerticalGapProperty()
                                                                                                                                       .get()
                                                                                                     : 0)
                                                    : 0;
   }

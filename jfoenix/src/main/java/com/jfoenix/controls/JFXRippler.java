...
/**
 * JFXRippler is the material design implementation of a ripple effect. the ripple effect can be
 * applied to any node in the scene. JFXRippler is a {@link StackPane} container that holds a
 * specified node (control node) and a ripple generator.
 * <p>
 * UPDATE NOTES: - fireEventProgrammatically(Event) method has been removed as the ripple controller
 * is the control itself, so you can trigger manual ripple by firing mouse event on the control
...
public class JFXRippler extends StackPane
{
    ...
    *
    * @param control
    * @param pos     can be either FRONT/BACK (position the ripple effect infront of or behind the
    *                control)
    */
   ...
   /**
    * show/hide the ripple overlay NOTE: setting overlay visibility to false will reset forceOverlay to
    * false
    *
    * @param visible
    */
   ...
   /**
    * this method will be set to private in future versions of JFoenix, user the method
    * {@link #setOverlayVisible(boolean)}
    */
   @Deprecated
   ...
   /**
    * Generates ripples on the screen every 0.3 seconds or whenever the createRipple method is called.
    * Ripples grow and fade out over 0.6 seconds
    */
   final class RippleGenerator extends Group
   {
      ...
      private void releaseRipple()
      {
         ...
         if (ripple != null)
         {
            ...
            ripple.outAnimation = new Timeline(new KeyFrame(Duration.millis(Math.min(800, (0.9 * 500) / ripple.getScaleX())), ripple.outKeyValues));
      ...
      void createOverlay()
      {
         if (overlayRect == null)
         {
            ...
            overlayRect.fillProperty().bind(Bindings.createObjectBinding(() ->
            {
               ...
               {
                  return new Color(((Color) ripplerFill.get()).getRed(), ((Color) ripplerFill.get()).getGreen(), ((Color) ripplerFill.get()).getBlue(), 0.2);
               }
               else
               {
      ...
      private final class OverLayRipple extends Rectangle
      {
         ...
         Animation inAnimation = new Timeline(new KeyFrame(Duration.millis(300), new KeyValue(opacityProperty(), 1, Interpolator.EASE_IN)));

         Animation outAnimation = new Timeline(new KeyFrame(Duration.millis(300), new KeyValue(opacityProperty(), 0, Interpolator.EASE_OUT)));
      ...
      private final class Ripple extends Circle
      {
         ...
         private Ripple(double centerX, double centerY)
         {
            super(centerX,
                  centerY,
                  ripplerRadius.get().doubleValue() == Region.USE_COMPUTED_SIZE ? computeRippleRadius() : ripplerRadius.get().doubleValue(),
                  null);
            setCache(true);
            setCacheHint(CacheHint.SPEED);
            setCacheShape(true);
            ...
            if (isRipplerRecenter())
            {
               ...
               inKeyValues[2] = outKeyValues[3] = new KeyValue(translateXProperty(),
                                                               Math.signum(dx) * Math.min(Math.abs(dx), this.getRadius() / 2),
                                                               rippleInterpolator);
               inKeyValues[3] = outKeyValues[4] = new KeyValue(translateYProperty(),
                                                               Math.signum(dy) * Math.min(Math.abs(dy), this.getRadius() / 2),
                                                               rippleInterpolator);
            }
   ...
   /**
    * the ripple recenter property, by default it's false. if true the ripple effect will show
    * gravitational pull to the center of its control
    */
   private StyleableObjectProperty<Boolean> ripplerRecenter = new SimpleStyleableObjectProperty<>(StyleableProperties.RIPPLER_RECENTER,
                                                                                                  JFXRippler.this,
                                                                                                  "ripplerRecenter",
                                                                                                  false);
    ...
    */
   private StyleableObjectProperty<Number> ripplerRadius = new SimpleStyleableObjectProperty<>(StyleableProperties.RIPPLER_RADIUS,
                                                                                               JFXRippler.this,
                                                                                               "ripplerRadius",
   ...
   /**
    * mask property used for clipping the rippler. can be either CIRCLE/RECT
    */
   private StyleableObjectProperty<RipplerMask> maskType = new SimpleStyleableObjectProperty<>(StyleableProperties.MASK_TYPE,
                                                                                               JFXRippler.this,
                                                                                               "maskType",
                                                                                               RipplerMask.RECT);
   ...
   /**
    * the ripple disable, by default it's false. if true the ripple effect will be hidden
    */
   private StyleableBooleanProperty ripplerDisabled = new SimpleStyleableBooleanProperty(StyleableProperties.RIPPLER_DISABLED,
                                                                                         JFXRippler.this,
                                                                                         "ripplerDisabled",
                                                                                         false);
   ...
   {
      private static final CssMetaData<JFXRippler, Boolean> RIPPLER_RECENTER = new CssMetaData<JFXRippler, Boolean>("-jfx-rippler-recenter",
      ...
      private static final CssMetaData<JFXRippler, Boolean> RIPPLER_DISABLED = new CssMetaData<JFXRippler, Boolean>("-jfx-rippler-disabled",
                                                                                                                    BooleanConverter.getInstance(),
                                                                                                                    false)
      {
         @Override
      ...
      private static final CssMetaData<JFXRippler, Paint> RIPPLER_FILL = new CssMetaData<JFXRippler, Paint>("-jfx-rippler-fill",
      ...
      private static final CssMetaData<JFXRippler, Number> RIPPLER_RADIUS = new CssMetaData<JFXRippler, Number>("-jfx-rippler-radius",
      ...
      private static final CssMetaData<JFXRippler, RipplerMask> MASK_TYPE = new CssMetaData<JFXRippler, RipplerMask>("-jfx-mask-type",
                                                                                                                     RipplerMaskTypeConverter.getInstance(),
                                                                                                                     RipplerMask.RECT)
      {
         @Override
         public boolean isSettable(JFXRippler control)

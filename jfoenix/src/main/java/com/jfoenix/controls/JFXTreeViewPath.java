...
public class JFXTreeViewPath extends ScrollPane
{
   ...
   public JFXButton createFirstButton(TreeItem temp)
   {
      return new JFXButton(temp.getValue().toString())
      {
         ...
         protected void layoutChildren()
         {
            ...
            Polygon polygon = new Polygon();
            final double height = getHeight();
            polygon.getPoints().addAll(new Double[] {0.0, 0.0, width - getOffset(), 0.0, width, height / 2, width - getOffset(), height, 0.0, height});
            setClip(polygon);
         }
      };
   }
   ...
   private JFXButton createLastButton(TreeItem temp, TreeItem parent)
   {
      return new JFXButton(temp.getValue().toString())
      {
         ...
         protected void layoutChildren()
         {
            ...
            Polygon polygon = new Polygon();
            final double height = getHeight();
            polygon.getPoints().addAll(new Double[] {0.0, 0.0, width, 0.0, width, height, 0.0, height, noParent ? 0 : getOffset(), noParent ? 0 : height / 2});
            setClip(polygon);
         }
      };
   }
   ...
   private StyleableDoubleProperty offset = new SimpleStyleableDoubleProperty(StyleableProperties.OFFSET, JFXTreeViewPath.this, "offset", 10.0);
   ...
   {
      private static final CssMetaData<JFXTreeViewPath, Number> OFFSET = new CssMetaData<JFXTreeViewPath, Number>("-jfx-offset",
      ...
      static
      {
         final List<CssMetaData<? extends Styleable, ?>> styleables = new ArrayList<>(ScrollPane.getClassCssMetaData());

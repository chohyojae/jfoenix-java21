...
public class JFXPopup extends PopupControl
{
   ...
   public void show(Node node, PopupVPosition vAlign, PopupHPosition hAlign, double initOffsetX, double initOffsetY)
   {
      if (!isShowing())
      {
         ...
         final double anchorX = parent.getX() + origin.getX() + node.getScene().getX() + (hAlign == PopupHPosition.RIGHT ? ((Region) node).getWidth() : 0);
         final double anchorY = parent.getY() + origin.getY() + node.getScene().getY() + (vAlign == PopupVPosition.BOTTOM ? ((Region) node).getHeight() : 0);

...
public class JFXDecorator extends VBox
{
   ...
   private double newX, newY, initX, initY, initWidth = -1, initHeight = -1, initStageX = -1, initStageY = -1;
   ...
   private ObjectProperty<Runnable> onCloseButtonAction = new SimpleObjectProperty<>(() -> primaryStage.fireEvent(new WindowEvent(primaryStage,
   ...
   /**
    * Create a window decorator for the specified node with the options: - full screen - maximize -
    * minimize
    *
   ...
   public JFXDecorator(Stage stage, Node node, boolean fullScreen, boolean max, boolean min)
   {
      ...
      primaryStage.fullScreenProperty().addListener((o, oldVal, newVal) ->
      {
         if (newVal)
         {
            ...
            /*
             * note the border property MUST NOT be bound to another property when going full screen mode, thus
             * the binding will be lost if exisited
             */
   ...
   /**
    * this property is used to replace JavaFX maximization with a custom one that prevents hiding
    ...
    * <pre>
    *     {@code
    * jfxDecorator.titleProperty().unbind();
    * }</pre>
    * 
    * first.
    */
   ...
   /**
    * If you want the {@code primaryStage}'s title and the {@code JFXDecorator}'s title to be
    * different, then go ahead and use this method.
    * <p>
    * By default, this title property is bound to the {@code primaryStage}'s title property-so merely
    * setting the {@code primaryStage}'s title, will set the {@code JFXDecorator}'s title.
    */

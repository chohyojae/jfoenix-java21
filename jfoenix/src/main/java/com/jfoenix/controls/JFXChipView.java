 ...
 * text area component with an x to omit the chip.
 *
...
public class JFXChipView<T> extends Control
{
   ...
   private ObjectProperty<BiPredicate<T, String>> predicate = new SimpleObjectProperty<>((item, text) ->
   {
   ...
   /**
    * Converts the user-typed input (when the ChipArea is editable to an object of type T, such that
    * the input may be retrieved via the property.
    */

   private ObjectProperty<StringConverter<T>> converter = new SimpleObjectProperty<StringConverter<T>>(this, "converter", JFXChipView.defaultStringConverter());

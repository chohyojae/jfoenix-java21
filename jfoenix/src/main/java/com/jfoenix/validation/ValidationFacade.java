...
/**
 * this class has been deprecated and will be removed in later versions of JFoenix, we are moving
 * validations into each control that implements the interface {@Link IFXValidatableControl}.
 * Validation will be applied through the control itself similar to
 * {@link com.jfoenix.controls.JFXTextField}, it's straight forward and simpler than using the
 * ValidationFacade.
 */
@Deprecated
public class ValidationFacade extends VBox
{
      ...
      errorLabel.heightProperty().addListener((o, oldVal, newVal) ->
      {
         if (errorShown)
         {
            ...
            errorAnimation.setOnFinished(finish -> new Timeline(new KeyFrame(Duration.millis(160),
                                                                             new KeyValue(errorContainer.opacityProperty(), 1, Interpolator.EASE_BOTH)))
                                                                                                                                                        .play());
   ...
   /**
    * list of validators that will validate the text value upon calling { {@link #validate()}
    */
